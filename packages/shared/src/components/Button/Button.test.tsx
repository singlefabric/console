...
describe('@kubed/components/Button', () => {
  checkAccessibility([mountWithTheme(<Button>Kubed button</Button>)]);
  ...
  it('passes type to button component', () => {
    const element = shallowWithTheme(<Button type="submit" />);
    expect(element.render().attr('type')).toBe('submit');
  });

  it('sets disabled attribute based on prop', () => {
    const disabled = shallowWithTheme(<Button disabled />);
    const notDisabled = shallowWithTheme(<Button />);
