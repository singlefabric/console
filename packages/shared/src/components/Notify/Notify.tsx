...
const getTypeColors = (type: string, theme: any): string => {
  const { palette } = theme;
  if (type === 'error') {
...
const IconWrap = styled('div')<WrapType>`
  ...
  min-width: 50px;
  height: 100%;
  ${({ type = 'blank', theme }) => `background-color: ${getTypeColors(type, theme)};`}
`;
...
const MessageWrap = styled('div')<WrapType>`
  ...
  .title-wrap {
    margin-bottom: 4px;
    ${({ type = 'blank', theme }) => `color: ${getTypeColors(type, theme)};`}
  }
`;
